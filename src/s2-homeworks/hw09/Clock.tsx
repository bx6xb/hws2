...
function Clock() {
  ...
  const start = () => {
    ...
    const intervalId = window.setInterval(() => setDate(new Date()), 1000)
  ...
  const stringTime = `${formatDate(date.getHours())}:${formatDate(date.getMinutes())}:${formatDate(
    date.getSeconds()
  )}` || <br /> // часы24:минуты:секунды (01:02:03)/(23:02:03)/(24:00:00)/(00:00:01) // пишут студенты
  const stringDate = `${formatDate(date.getDate())}.${formatDate(date.getMonth() + 1)}.${formatDate(
    date.getFullYear()
